...
describe("WinHandler", () => {
  ...
  describe("getEmulatorConfigPath & getConfigDir", () => {
    it("should return correct path for dolphin", () => {
      ...
      expect(handler.getConfigDir("dolphin")).toBe(expected);
    });
    ...
    it("should throw for unsupported engines", () => {
      expect(() => handler.getEmulatorConfigPath("unsupported" as EngineID)).toThrow(
        "[Win] Emulator config path not found for: unsupported"
      );
    });
  });
  ...
  describe("getEmulatorBasePath", () => {
    ...
    it("should throw for unsupported engines", () => {
      expect(() => handler.getEmulatorBasePath("unsupported" as EngineID)).toThrow(
        "[Win] Emulator base path not found for: unsupported"
      );
    });
  });

  describe("getSavePath & getSaveDir", () => {
    ...
    it("should throw for unknown engineId", () => {
      ...
      expect(() => handler.getSavePath(game)).toThrow("[SaveService] Unknown engine: unknown");
    });
  });

  describe("getBiosDir", () => {
    it("should return correct BIOS path for dolphin", () => {
      expect(handler.getBiosDir("dolphin")).toBe(
        path.join("C:\\Users\\TestUser", "AppData", "Roaming", "Dolphin Emulator", "Sys")
      );
    });

    it("should return correct BIOS path for mesen", () => {
      expect(handler.getBiosDir("mesen")).toBe(
        path.join("C:\\Users\\TestUser", "AppData", "Roaming", "Mesen2", "Firmware")
      );
    });

    it("should return correct BIOS path for ares", () => {
      expect(handler.getBiosDir("ares")).toBe(
        path.join("C:\\Users\\TestUser", "AppData", "Local", "ares", "Firmware")
      );
    });

    it("should return correct BIOS path for melonds", () => {
      expect(handler.getBiosDir("melonds")).toBe(
        path.join("C:\\Users\\TestUser", "AppData", "Local", "melonDS")
      );
    });

    it("should return correct BIOS path for azahar", () => {
      expect(handler.getBiosDir("azahar")).toBe(
        path.join("C:\\Users\\TestUser", "AppData", "Roaming", "Azahar", "sysdata")
      );
    });

    it("should return correct BIOS path for pcsx2", () => {
      expect(handler.getBiosDir("pcsx2")).toBe(
        path.join("C:\\Users\\TestUser", "Documents", "PCSX2", "bios")
      );
    });

    it("should return correct BIOS path for duckstation", () => {
      expect(handler.getBiosDir("duckstation")).toBe(
        path.join("C:\\Users\\TestUser", "Documents", "DuckStation", "bios")
      );
    });

    it("should throw for unsupported engines", () => {
      expect(() => handler.getBiosDir("unsupported" as EngineID)).toThrow(
        "[Win] BIOS dir not found for: unsupported"
      );
    });
  });
