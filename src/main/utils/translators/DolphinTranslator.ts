...
export class DolphinTranslator implements IEmulatorTranslator {
  ...
  translate(profile: ControlsProfile, ctx: TranslateContext): EmulatorPatch[] {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (ctx && i === 0) ctx.learnedBinds = info.learnedBinds;
      
      let device = info.deviceString;
      if (kind === "gamepad" && ctx.learnedDevice && i === 0) {
      ...
      if (which === "wii") {
        ...
        let sideways = "False";
        if (ctx.controllerId === "wiimote_nunchuk") extension = "Nunchuk";
        if (ctx.controllerId === "wiimote" || ctx.controllerId === "wiimote_sideways") extension = "None";
        if (ctx.controllerId === "wiimote_sideways") sideways = "True";
