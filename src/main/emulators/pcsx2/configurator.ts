...
import { PCSX2Translator } from "./translator";
import type { TranslateContext } from "../translatorTypes";
import { PCSX2 } from "./schema";
...
export class PCSX2Configurator extends BaseConfigurator {
  async configure(): Promise<void> {
    ...
    const configDir = osHandler.getEmulatorBasePath("pcsx2");
    ...
    const biosFile = findPlayStationBios(biosDir, PCSX2_VALID_BIOS);
    ...
    const { layout, effectiveProfile } = await this.resolveControls("ps2");
