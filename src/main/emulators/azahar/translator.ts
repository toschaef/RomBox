...
import { digitalToGamepadToken, getDirFromBinding } from "../../utils/profileRead";
import { AZAHAR, azProfileKey, azProfileDefaultKey, qtKeycodeFromDomCode } from "./schema";
import type { AzaharLearnedSDL } from "./sdlProbe";
...
export class AzaharTranslator implements IEmulatorTranslator {
  id = "azahar";

  /** `learned` is what the SDL probe reported for the attached controller. */
  constructor(private learned: AzaharLearnedSDL | null) {}

  translate(profile: ControlsProfile, _ctx: TranslateContext): EmulatorPatch[] {
    return this.translateFromPlayer(profile.player1);
  }

  /** Azahar only ever configures player 1. */
  private translateFromPlayer(bindings: PlayerBindings): EmulatorPatch[] {
