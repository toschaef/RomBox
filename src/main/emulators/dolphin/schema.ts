...
export function dolphinExprForGamepadToken(tok: GamepadToken, platform = "win32", learnedBinds?: LearnedBinds): string | null {
  ...
  if (learnedBinds && platform === "darwin") {
    ...
    if (sdlBtn != null) {
      ...
      if (binding?.kind === "button" && binding.button !== -1) {
...
): { deviceString: string; learnedBinds?: LearnedBinds } {
