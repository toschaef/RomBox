...
export function plansEqual(a: BindPlan | null | undefined, b: BindPlan | null | undefined): boolean {
  if (!a || !b) return false;
  if (a.kind !== b.kind) return false;
  if (a.kind === "digital") return a.path === (b as Extract<BindPlan, { kind: "digital" }>).path;
  if (a.kind === "dpad") return a.group === (b as Extract<BindPlan, { kind: "dpad" }>).group;
  const bs = b as Extract<BindPlan, { kind: "stick" }>;
  return a.group === bs.group && a.stick === bs.stick;
}
