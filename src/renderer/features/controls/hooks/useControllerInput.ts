...
/** keys the page would otherwise scroll with while someone is binding */
...
export function useControllerInput(options?: { paused?: boolean }) {
  ...
  // drop any held input so nothing stays highlighted while paused
  useEffect(() => {
    if (!paused) return;
    keyboardDown.current.clear();
    prevPressed.current = new Set();
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (SCROLL_KEYS.has(e.code) && !isTextTarget(e.target)) e.preventDefault();
